...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (code === "USER_NOT_FOUND") nav("/signup", { state: { email } });
      else alert(code || "LOGIN_FAILED");
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen grid place-items-center px-4">
        <div className="w-full max-w-sm rounded-3xl bg-white/10 backdrop-blur-md border border-white/20 shadow-2xl p-6">
          ...
          <p className="text-sm text-white/70 text-center mt-1">Login to ECE Cricket</p>

          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label className="block text-sm text-white/80 mb-1">Email</label>
              <input
                className="w-full rounded-xl bg-white/10 text-white placeholder-white/60 border border-white/20 focus:border-emerald-400 focus:ring-0 px-4 py-3 outline-none"
            ...
            <button
              className="w-full rounded-xl bg-gradient-to-r from-emerald-400 to-teal-500 text-black font-semibold py-3 hover:brightness-110 transition disabled:opacity-60"
              disabled={loading}
            >
